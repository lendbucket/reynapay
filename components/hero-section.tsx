    ...
    <div className="relative">
      <div className="relative bg-[var(--color-card)] rounded-[var(--radius-xl)] shadow-[var(--shadow-lg)] border border-[var(--color-border)] overflow-hidden">
        <div className="bg-[var(--color-surface-2)] border-b border-[var(--color-border)] p-3 flex items-center gap-2">
          ...
          <div className="ml-3 text-xs text-[var(--color-ink-subtle)] font-mono">app.reynapay.com/dashboard</div>
        </div>
        <div className="p-6 space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-[var(--color-ink-subtle)] uppercase tracking-wider">Today's volume</div>
              <div className="text-3xl font-bold tracking-tight mt-1">$12,847.50</div>
              <div className="text-xs text-[var(--color-success)] font-medium mt-1">↑ 18.2% vs. yesterday</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-[var(--color-ink-subtle)]">Effective rate</div>
              <div className="text-lg font-semibold text-[var(--color-brand)]">2.31%</div>
            </div>
          </div>
          <div className="h-24 bg-gradient-to-t from-[var(--color-brand-tint)] to-transparent rounded-[var(--radius-md)] relative">
            <svg viewBox="0 0 400 100" className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
              <path d="M0,80 Q50,60 100,55 T200,40 T300,30 T400,15" stroke="var(--color-brand)" strokeWidth="2" fill="none" />
              <path d="M0,80 Q50,60 100,55 T200,40 T300,30 T400,15 L400,100 L0,100 Z" fill="var(--color-brand)" opacity="0.08" />
            </svg>
          </div>
...
function TxRow({ merchant, amount }: { merchant: string; amount: string }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-[var(--color-border)] last:border-0">
      <span className="text-xs text-[var(--color-ink-muted)] truncate pr-3">{merchant}</span>
