    ...
    <>
        ...
        >
          ...
          <div className="relative container-page">
            <div className="max-w-3xl mx-auto text-center text-[var(--color-accent)]">
              ...
              <h1 className="mb-5 text-balance text-[var(--color-accent)]">
            ...
            {content.problem.bullets && content.problem.bullets.length > 0 && (
              <ul className="space-y-3 text-left max-w-2xl mx-auto">
                {content.problem.bullets.map((b, i) => (
                  ...
                  >
                    <span className="text-[var(--color-brand)] font-bold mt-0.5 flex-shrink-0">
                      →
                    </span>
                    <span>{b}</span>
                  </li>
      ...
      {content.outcomes && (
        <Section variant="brand">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h2 className="mb-5 text-balance text-[var(--color-accent)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {content.outcomes.stats.map((s, i) => (
                <div key={i} className="text-center">
                  ...
                  <div className="text-sm text-[var(--color-accent)]/80 leading-snug">
                    {s.label}
                  </div>
                </div>
              ))}
      ...
      <Section variant="soft">
        <SectionHeader
          ...
          title={`Features built for ${ind.name.toLowerCase()}.`}
        />
              ...
              >
                ...
                >
                  <h3 className="text-2xl md:text-3xl mb-4 font-semibold leading-tight flex items-start gap-3">
                    <CheckCircle2
                      size={24}
                      className="text-[var(--color-brand)] flex-shrink-0 mt-1"
                    />
                    {f.title}
                  </h3>
                  <p className="text-[var(--color-ink-muted)] text-lg leading-relaxed pl-9">
                    {f.description}
                  </p>
                </div>
              </div>
      ...
      {content.useCases && content.useCases.length > 0 && (
        <Section variant="default">
          <SectionHeader
            ...
            title={`How ${ind.name.toLowerCase()} actually use Reyna Pay.`}
          />
        ...
        <Section variant="soft">
          <div className="max-w-3xl mx-auto">
            <Card className="!p-8 md:!p-12 relative">
              <Quote
                size={48}
                className="text-[var(--color-brand)]/20 absolute top-6 left-6"
                aria-hidden
              />
              ...
              <div className="pl-12">
                ...
                {content.testimonial.isPlaceholder && (
                  <div className="mt-3 text-[10px] text-[var(--color-warning)] uppercase tracking-wider">
          ...
          return (
            <Section variant="default">
              <div className="max-w-2xl mx-auto text-center">
                ...
                <p className="text-[var(--color-ink-muted)] mb-8 leading-relaxed text-lg">
                  {product.description}
                </p>
                {external ? (
      ...
      <Section variant="deep">
        <SectionHeader
          eyebrow="FAQ"
          title={`Common questions for ${ind.name.toLowerCase()}.`}
        />
        <div className="max-w-3xl mx-auto">
      ...
      {relatedSolutions.length > 0 && (
        <Section variant="default">
          <SectionHeader
            ...
            title="Solutions that power this industry."
          />
      ...
      {relatedIndustries.length > 0 && (
        <Section variant="soft">
          <SectionHeader
            ...
            title="Other verticals we serve."
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto">
            {relatedPosts.map((p) => (
              <Link
                key={p.slug}
                href={`/blog/${p.slug}`}
                className="group card-base p-6 flex flex-col"
              >
                <h3 className="text-base mb-2 font-semibold group-hover:text-[var(--color-brand)] transition-colors leading-snug">
                  {p.title}
                </h3>
                <p className="text-xs text-[var(--color-ink-muted)] leading-relaxed flex-1">
                  {p.excerpt}
                </p>
