...
const whyFeatures = [
  {
    ...
    description: "Every transaction itemizes interchange, network assessments, and our markup separately. No hidden padding.",
  },
  {
    icon: Sparkles,
    title: "Vertical-specific products",
    description: "Salons get SalonTransact. Restaurants get KasseApp. Each product is built around how that industry actually operates.",
  },
  {
    icon: HeartHandshake,
    title: "Real human support",
    description: "Dedicated account reps. Real engineers fixing real problems. Support phones answered by people, not chatbots.",
  },
  {
    icon: Zap,
      ...
      <HeroSection
        ...
        headline={
          <>
            Payment processing,
            <br />
            <span className="text-[var(--color-brand)]">built for your industry.</span>
          </>
        }
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {PRODUCTS.map((p) => (
            ...
            >
              ...
              <h3 className="text-xl mb-3 font-semibold group-hover:text-[var(--color-brand)] transition-colors">{p.name}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
          <div className="lg:col-span-1">
            ...
            <p className="text-[var(--color-ink-muted)] leading-relaxed">
              Reyna Pay operates on bank-grade rails. PCI DSS Level 1 compliant processing, card-network certified, audited annually.
            </p>
          </div>
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <Lock size={22} strokeWidth={1.75} className="text-[var(--color-brand)] mb-3" aria-hidden />
              <h3 className="text-base mb-1.5">PCI DSS Level 1</h3>
              <p className="text-sm text-[var(--color-ink-muted)]">Highest level of payment card security compliance.</p>
            </Card>
            <Card>
              <ShieldCheck size={22} strokeWidth={1.75} className="text-[var(--color-brand)] mb-3" aria-hidden />
              <h3 className="text-base mb-1.5">Network certified</h3>
              <p className="text-sm text-[var(--color-ink-muted)]">Direct relationships with Visa, Mastercard, Amex, Discover.</p>
            </Card>
            <Card>
              <Building2 size={22} strokeWidth={1.75} className="text-[var(--color-brand)] mb-3" aria-hidden />
              <h3 className="text-base mb-1.5">Bank-sponsored</h3>
              <p className="text-sm text-[var(--color-ink-muted)]">Settlement through FDIC-insured sponsor banks.</p>
            </Card>
          </div>
        </div>
      </Section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-5xl mx-auto">
          <Card className="!p-10">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-[var(--radius-md)] bg-[var(--color-brand-tint)] mb-5">
              <Users size={22} strokeWidth={1.75} className="text-[var(--color-brand)]" aria-hidden />
            </div>
            <h3 className="text-2xl mb-3 font-semibold">For business owners</h3>
            <p className="text-[var(--color-ink-muted)] mb-6 leading-relaxed">
              Apply once, get approved, start accepting cards with a vertical-specific platform that fits how your business actually runs.
            </p>
            ...
            <Button href="/apply">Apply as a business <ArrowRight size={14} /></Button>
          </Card>
          <Card className="!p-10">
            <div className="inline-flex items-center justify-center w-12 h-12 rounded-[var(--radius-md)] bg-[var(--color-brand-tint)] mb-5">
              <HeartHandshake size={22} strokeWidth={1.75} className="text-[var(--color-brand)]" aria-hidden />
            </div>
            <h3 className="text-2xl mb-3 font-semibold">For partners</h3>
            <p className="text-[var(--color-ink-muted)] mb-6 leading-relaxed">
              Bring us merchants. We provide the infrastructure, compliance, and support. You get a branded portal and recurring commission on every transaction.
            </p>
            <ul className="space-y-2.5 mb-8">
            ...
            <Button href="/partners" variant="secondary">Learn about the partner program <ArrowRight size={14} /></Button>
          </Card>
        </div>
      </Section>
...
function CheckRow({ children }: { children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-2.5 text-[0.9375rem] text-[var(--color-ink-muted)]">
      <CheckCircle2 size={18} className="text-[var(--color-brand)] flex-shrink-0 mt-0.5" aria-hidden />
      <span>{children}</span>
    </li>
  );
}
