...
export const metadata = buildMetadata({
  ...
  description: "Card-on-file, no-show fees, stylist commission tracking, multi-location reporting, white-label payments, and more. The capabilities that power Reyna Pay's vertical products.",
  path: "/solutions",
});
...
export default function SolutionsPage() {
  return (
    <>
      ...
      <HeroSection
        eyebrow="Solutions"
        headline={<>The capabilities that <span className="text-[var(--color-brand)]">power every Reyna Pay product.</span></>}
      ...
      <Section>
        <SectionHeader eyebrow="All solutions" title="Browse what's possible." centered={false} />
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {SOLUTIONS.map((s) => (
            <Link key={s.slug} href={`/solutions/${s.slug}`} className="group card-base p-7 flex flex-col">
              <h3 className="text-lg mb-2 font-semibold group-hover:text-[var(--color-brand)] transition-colors">{s.name}</h3>
              <p className="text-sm text-[var(--color-ink-muted)] leading-relaxed flex-1">{s.description}</p>
